...
const searchRestaurant = async (req: Request, res: Response) => {
  try {
    ...
    query["city"] = new RegExp(city, "i"); //RegExp = ignore the case sensitive

    // take the number of records that match to the city
    const cityCheck = await Restaurant.countDocuments(query);

    // if no output | no point on going forward
    if (cityCheck === 0) {
    ...
    res
      .status(500)
      .json({ message: "Something went wrong with searchRestaurant." });
  }
};
